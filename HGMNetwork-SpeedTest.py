...
# Special Integrated To: HGMNetwork.V5
# All Rights Reserved.
#
...
class SpeedtestResults(object):
    ...
    def share(self):
        ...
        self._share = 'http://www.speedtest.net/result/%s' % resultid[0]
...
def shell():
    ...
    printer('###############################################\nSpeedTest.Net Konfigurasyonu Aliniyor...', quiet)
    try:
        speedtest = Speedtest(
        ...
        printer('###############################################\nSpeedTest.Net Konfigurasyonu Alinamadi..!\n###############################################', error=True)
    ...
    if args.list:
        try:
            ...
            printer('Sunucu Listesi Alinamadi..!', error=True)
            raise SpeedtestCLIError(get_exception())
    ...
    printer('Sunucu Ip/Host %(isp)s (%(ip)s)...' % speedtest.config['client'],
            quiet)

    if not args.mini:
        printer('SpeedTest.Net Sunucu Listesi Aliniyor...', quiet)
        try:
            ...
            printer('SpeedTest.Net Sunucu Listesi Alinamadi..!', error=True)
            raise SpeedtestCLIError(get_exception())
        ...
        if args.server and len(args.server) == 1:
            printer('Secilen Sunucu Bilgileri Aliniyor...', quiet)
        else:
            printer('En Iyi Test Sunucusu Araniyor...', quiet)
    ...
    printer('Test Sunucu: %(sponsor)s (%(name)s) [%(d)0.2f km]: '
            '%(latency)s Ms\n###############################################' % results.server, quiet)

    if args.download:
        printer('\n###############################\n## Indirme Hizi Olculuyor...\n###############################', quiet,
                end=('', '\n')[bool(debug)])
        speedtest.download(
            callback=callback,
            threads=(None, 1)[args.single]
        )
        printer('##########################################\n## Indirme Hizi: %0.2f M%s // Saniye\n##########################################' %
